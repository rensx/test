...
public class PxScene {
    ...
    private final List<PxAnim> pendingAnimations = new ArrayList<>(); // Temporary list for new animations
    ...
    public PxScene(int w, int h) {
        ...
        this.physicsWorld.getWorld().setGravity(0, -9.81);
    }
    ...
    public void update(double dt) {
        ...
        List<PxAnim> animationsToUpdate = new ArrayList<>(animations);
        for (PxAnim anim : animationsToUpdate) {
            anim.update(dt);
        }
        // Add pending animations after iteration
    ...
    public void playAnimation(PxAnim... anims) {
        for (PxAnim anim : anims) {
            if (anim != null) {
                anim.setScene(this);
                pendingAnimations.add(anim); // Add to pending list
            }
        }
    }

    public void advanceFrame() {
        for (PxAnim anim : new ArrayList<>(animations)) {
    ...
    public PVector worldToScreen(PVector world) {
        float sx = (world.x - viewCenter.x) * viewScale + width / 2f;
        float sy = height / 2f - (world.y - viewCenter.y) * viewScale;
        return new PVector(sx, sy);
    }

    public PVector screenToWorld(PVector screen) {
        float x = (screen.x - width / 2f) / viewScale + viewCenter.x;
        float y = (height / 2f - screen.y) / viewScale + viewCenter.y;
